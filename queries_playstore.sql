...
SELECT category, AVG(rating) AS rates FROM analytics GROUP BY category ORDER BY rates DESC;
...
app_name FROM analytics WHERE rating < 3 AND reviews>10000;
...
SELECT app_name, reviews, min_installs, (min_installs/reviews) AS propotion FROM analytics WHERE min_installs>100000;
...
SELECT app_name, genres FROM analytics
    WHERE array_length(analytics.genres, 1) > 1;
